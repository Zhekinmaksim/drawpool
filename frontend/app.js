...
function parseChainError(err, context = {}) {
  const candidates = [
    err?.data,
    err?.info?.error?.data,
    err?.error?.data,
  ...
  for (const raw of candidates) {
    try {
      const parsed = pool?.interface?.parseError?.(raw);
      if (!parsed) continue;
      switch (parsed.name) {
        case "TooEarly":
          return context.secondsUntilDraw > 0
            ? `Too early, next draw opens in ${humanEta(context.secondsUntilDraw)}`
            : "Too early";
        case "NothingDeposited":
          return "No deposits in the pool yet";
        case "DrawAlreadyActive":
          return "A draw is already active";
        case "RandomnessNotReady":
          return "Committed block not finalized yet";
        case "NoActiveDraw":
          return "No active draw to settle";
        case "NoPrizeYet":
          return "No yield prize available yet";
        case "PoolLocked":
          return "Pool is locked during an active draw";
        default:
          return parsed.name;
      }
    } catch {}
...
function startDrip() {
  ...
  dripTimer = setInterval(() => {
    if (document.hidden || drawRunning || liveMode) return;
...
function commitPhase(commitBlockOverride = null) {
  ...
  // commit data
  committedBlock = commitBlockOverride ?? (liveMode ? 0 : 4820000 + (Math.random() * 9000 | 0));
...
function awardPhase(participants, forcedWinnerName = null, forcedHash = null) {
  if (!drawRunning) { toast("Commit the draw first"); return; }
  // pick weighted winner (honest: odds ∝ balance)
  let winner;
  if (forcedWinnerName) {
    winner = participants.find((p) => p.name === forcedWinnerName) || { name: forcedWinnerName, bal: 0 };
  } else {
  ...
  clearInterval(hashScrambler);
  const finalHash = forcedHash || randHash();
...
async function loadLiveParticipantsForReel() {
  const n = Number(await pool.participantCount());
  const parts = [];
  for (let i = 0; i < n; i++) {
    const addr = await pool.participantAt(i);
    const bal = await pool.balanceOf(addr);
    parts.push({ name: shortAddr(addr), bal: Number(ethers.formatUnits(bal, DEC)) });
  }
  return parts.sort((a, b) => b.bal - a.bal);
}
...
async function refresh() {
  if (!pool) return;
  const [total, parts, drawCount, prize, bal, odds, wbal, nextAt, active, hLen, latestBlock] = await Promise.all([
    ...
    pool.drawActive(), pool.historyLength(), provider.getBlock("latest")]);
  ...
  const secondsUntilDraw = Math.max(0, Number(nextAt) - chainNow);
  if (!drawRunning) {
    $("st1").classList.toggle("on", active); $("st2").classList.toggle("on", active); $("st3").classList.toggle("on", !active);
    $("startBtn").disabled = active || secondsUntilDraw > 0;
    $("awardBtn").disabled = !active;
    $("runHint").textContent = active
      ? "Draw committed. Once the block finalizes, anyone can settle the yield payout."
      : secondsUntilDraw > 0
        ? `Next draw opens in ${humanEta(secondsUntilDraw)}`
  ...
  renderWinners(rows);
  paintCountdown(secondsUntilDraw);
}
...
async function claimFaucetFlow() {
  if (!cfg || !ABIS) { toast("Demo mode, the faucet works once live contracts are wired."); return; }
  if (!liveMode) {
    try { await connect(); }
    catch (e) { console.error(e); toast((e.shortMessage || e.message || "Wallet connection failed").slice(0, 90)); return; }
  }
  if (!usdc) { toast("Wallet connection required."); return; }
  return tx(usdc.faucet(), "Claiming faucet");
}
$("heroFaucet").onclick = claimFaucetFlow;
$("faucetBtn").onclick = claimFaucetFlow;
...
$("startBtn").onclick = () => {
  if (liveMode) return (async () => {
    try {
      toast("Committing draw…");
      const t = await pool.startDraw();
      await t.wait();
      const draw = await pool.currentDraw();
      const commitBlock = await rng.commitBlockOf(cfg.contracts.DrawPool, draw.id);
      commitPhase(Number(commitBlock));
      toast("Committing draw ✓");
      await refresh();
    } catch (e) {
      console.error(e);
      try {
        ...
        const nextAt = await pool.nextDrawAt();
        const secondsUntilDraw = Math.max(0, Number(nextAt) - Number(latestBlock.timestamp));
        toast(parseChainError(e, { secondsUntilDraw }));
      } catch {
        toast(parseChainError(e));
      }
    }
  })();
  commitPhase();
...
$("awardBtn").onclick = () => {
  if (liveMode) return (async () => {
    try {
      toast("Settling draw…");
      const t = await pool.award();
      await t.wait();
      const hLen = await pool.historyLength();
      const last = await pool.history(hLen - 1n);
      const participants = await loadLiveParticipantsForReel();
      const winnerName = shortAddr(last.winner);
      const finalHash = "0x" + last.randomness.toString(16).padStart(64, "0");
      awardPhase(participants, winnerName, finalHash);
      toast("Settling draw ✓");
      await refresh();
    } catch (e) {
      console.error(e);
      toast((e.shortMessage || e.message || "Transaction failed").slice(0, 90));
    }
  })();
